...
public class WorkDaoImpl implements WorkDaoEx {
    ...
    public Page<Map<String,Object>> List(List<Long> departmentIds, List<Long> userIds, String name, List<Long> userTypeId, LocalDateTime startDateTime, LocalDateTime endDateTime, LionPage lionPage) {
        ...
        sb.append("select t1.id as id ,t1.name as name,t2.start_work_time as start_work_time,t2.end_work_time as end_work_time from [upms].[dbo].[t_user] t1 left join [manage].[dbo].[t_work] t2 on t1.id = t2.user_id  join [manage].[dbo].[t_department_user] t3 on t1.id = t3.user_id where 1=1 ");
        if (Objects.nonNull(departmentIds) && departmentIds.size()>0) {
            sb.append(" and t3.department_id in (:departmentId)");
